...
const generateRandomAuthor = (id) => {
  return {
    ...
    deathDate: Math.random() > 0.3 ? `20${Math.floor(Math.random() * 20)}-0${Math.floor(Math.random()*9)+1}-${Math.floor(Math.random()*20)+10}` : null,
  };
};
...
const AuthorsPage = () => {
    ...
    if (savedAuthors) {
      ...
      const sampleAuthors = [
        ...
        },
        {
          id: '3',
          firstName: 'F. Scott',
          lastName: 'Fitzgerald, un auteur américain connu pour ses romans sur l\'âge du jazz',
          bookCount: 1,
          birthDate: '1896-09-24',
          deathDate: '1940-12-21'
        }
      ];
      const generatedAuthors = Array.from({ length: 50 }, (_, i) => generateRandomAuthor(i + 4));
      const allAuthors = [...sampleAuthors, ...generatedAuthors];
      setAuthors(allAuthors.sort((a,b) => `${a.lastName} ${a.firstName}`.localeCompare(`${b.lastName} ${b.firstName}`)));
  ...
  const handleAddAuthor = (newAuthor) => {
    const updatedAuthors = [...authors, newAuthor].sort((a,b) => `${a.lastName} ${a.firstName}`.localeCompare(`${b.lastName} ${b.firstName}`));
  ...
  const handleLetterScroll = (letter) => {
    const firstAuthorWithLetter = filteredAuthors.find(author => author.lastName.toUpperCase().startsWith(letter));
    if (firstAuthorWithLetter && authorRefs.current[firstAuthorWithLetter.id]) {
      authorRefs.current[firstAuthorWithLetter.id].scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
    } else {
      toast({ title: "Aucun auteur", description: `Aucun auteur dont le nom commence par ${letter}.`, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="flex">
      <div className="flex-grow space-y-8 pr-16"> {/* Adjusted pr for AlphabeticalScroller width */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center"
        >
          <h1 className="text-4xl font-bold main-title-text mb-2">Gestion des Auteurs</h1>
          <p className="text-muted-foreground text-lg">Découvrez et gérez vos auteurs favoris</p>
        </motion.div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredAuthors.length > 0 ? (
            filteredAuthors.map((author, index) => (
              <div key={author.id} ref={el => authorRefs.current[author.id] = el} className="h-full">
                <AuthorCard author={author} index={index} onClick={() => setSelectedAuthor(author)} />
              </div>
            ))
          ) : (
            <motion.div
            ...
            >
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">Aucun auteur trouvé</h3>
              <p className="text-muted-foreground">
                {searchTerm ? 'Aucun auteur ne correspond à votre recherche.' : 'Commencez par ajouter votre premier auteur !'}
              </p>
            </motion.div>
          )}
        </div>
