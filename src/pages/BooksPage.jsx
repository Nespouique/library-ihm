...
const BooksPage = ({ initialSearchTerm }) => {
    ...
    if (savedBooks) {
      ...
      const sampleBooks = [
        ...
        },
        {
          ...
          isbn: '978-0743273565',
          description: 'Un roman sur le rêve américain et la décadence des années 20.',
      ...
      const generatedBooks = Array.from({ length: 50 }, (_, i) => generateRandomBook(i + 4, authorsData.length > 0 ? authorsData : [{firstName: "Auteur", lastName: "Aléatoire"}]));
  ...
  const handleAddBook = (newBook) => {
    const updatedBooks = [...books, newBook].sort((a, b) => a.title.localeCompare(b.title));
  ...
  const handleUpdateBook = (updatedBook) => {
    ...
    setBooks(updatedBooks);
    localStorage.setItem('library-books', JSON.stringify(updatedBooks));
    toast({ title: "Livre mis à jour!", description: `${updatedBook.title} a été mis à jour.` });
  };
  ...
  const handleLetterScroll = (letter) => {
    const firstBookWithLetter = filteredBooks.find(book => book.title.toUpperCase().startsWith(letter));
    if (firstBookWithLetter && bookRefs.current[firstBookWithLetter.id]) {
      bookRefs.current[firstBookWithLetter.id].scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
    } else {
      toast({ title: "Aucun livre", description: `Aucun livre ne commence par la lettre ${letter}.`, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="flex">
      <div className="flex-grow space-y-8 pr-16"> {/* Adjusted pr for AlphabeticalScroller width */}
        ...
        >
          <h1 className="text-4xl font-bold main-title-text mb-2">Gestion des Livres</h1>
          <p className="text-muted-foreground text-lg">Découvrez et gérez votre collection de livres</p>
        </motion.div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredBooks.length > 0 ? (
            filteredBooks.map((book, index) => (
              <div key={book.id} ref={el => bookRefs.current[book.id] = el} className="h-full">
            ...
            >
              ...
              <p className="text-muted-foreground">
                {searchTerm ? 'Aucun livre ne correspond à votre recherche.' : 'Commencez par ajouter votre premier livre !'}
              </p>
            </motion.div>
          )}
        </div>
