...
const ShelvesPage = () => {
  ...
  useEffect(() => {
    ...
    if (savedShelves) {
      ...
      const sampleShelves = [
        { id: '1', name: 'Fiction et Romans Modernes Internationaux', bookCount: 1 },
        { id: '2', name: 'Science-Fiction Dystopique', bookCount: 1 },
        { id: '3', name: 'Histoire Ancienne et Médiévale', bookCount: 0 },
        { id: '4', name: 'Classiques de la Littérature Mondiale', bookCount: 1 },
      ];
      setShelves(sampleShelves);
      localStorage.setItem('library-shelves', JSON.stringify(sampleShelves));
    }
  ...
  const handleAddShelf = (newShelf) => {
    const updatedShelves = [...shelves, newShelf].sort((a,b) => a.name.localeCompare(b.name));
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h1 className="text-4xl font-bold main-title-text mb-2">Gestion des Étagères</h1>
        <p className="text-muted-foreground text-lg">Organisez votre bibliothèque par étagères</p>
      </motion.div>
        ...
        {filteredShelves.length > 0 ? (
          filteredShelves.map((shelf, index) => (
            <div key={shelf.id} className="h-full">
              <ShelfCard shelf={shelf} index={index} onClick={() => setSelectedShelf(shelf)} />
            </div>
          ))
        ) : (
          <motion.div
          ...
          >
            ...
            <p className="text-muted-foreground">
              {searchTerm ? 'Aucune étagère ne correspond à votre recherche.' : 'Commencez par ajouter votre première étagère !'}
            </p>
          </motion.div>
        )}
      </div>
